...
namespace GlyssenTests.Dialogs
{
	...
	class BlockNavigatorViewModelTests
	{
		...
		{
			Assert.IsTrue(m_testProject.IncludedBooks.SelectMany(b => b.Blocks).Any(b => b.CharacterIsUnknown && b.MultiBlockQuote == MultiBlockQuote.Continuation),
		...
		public void SetCurrentBlockIndexInBook_BlockIsQuoteContinuationBlockForRelevantQuote_StateReflectsQuoteStartBlock()
		{
			...
			for (; i < blocks.Count; i++)
			{
				...
				{
					var quoteStart = i - 1;
					if (blocks[quoteStart].CharacterIsUnknown)
						break;
					do
					{
						i++;
