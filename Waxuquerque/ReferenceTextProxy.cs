	...
	public class ReferenceTextProxy : IReferenceTextProxy
	{
		...
		private static void LoadAllAvailable()
		{
			...
			if (firstLoadError != null)
			{
				...
				{
					throw new Exception(
						String.Format(Localizer.GetString("ReferenceText.NoReferenceTextsLoaded",
				...
				if (additionalErrors.Any())
				{
					ErrorReport.ReportNonFatalExceptionWithMessage(firstLoadError.Item1,
						String.Format(Localizer.GetString("ReferenceText.MultipleLoadErrors",
		...
		{
			return String.Format(Localizer.GetString("ReferenceText.CouldNotLoad", "The {0} reference text could not be loaded from: {1}"),
				token, path);
		}
