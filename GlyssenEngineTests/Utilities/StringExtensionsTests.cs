...
namespace GlyssenEngineTests.Utilities
{
	...
	public class StringExtensionsTests
	{
		...
		public void Truncate_ShorterOrEqualToLengthSpecified_NoChange(string input)
		{
			...
			Assert.That(input, Is.EqualTo(input.Truncate(maxLen)));
			Assert.That(input, Is.EqualTo(input.Truncate(maxLen + 1)));
		}
		...
		public void Truncate_TruncatingAndAddingEllipsesWouldMakeSameOrLonger_NoChange(string input, string ellipses)
		{
			...
			Assert.That(input, Is.EqualTo(input.Truncate(maxLen, ellipses)));
			for (int i = ellipses.Length - 1; i > 0; i--)
				Assert.That(input, Is.EqualTo(input.Truncate(maxLen - i, ellipses)));
		}
		...
		{
			Assert.That(input.Truncate(truncateAt, ellipses), Is.EqualTo("This is a super-duper very long string, my friend."));
		}
	}
}
