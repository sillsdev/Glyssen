...
//			File.WriteAllText("..\\..\\Resources\\temporary\\control_plus_FCBH.txt", sb.ToString());
		...
		private static void CompareAndCombineLists(bool includeFcbhNarrator, StringBuilder sb)
		{
			...
			var controlDictionary = new Dictionary<BCVRef, List<Waxuquerque.Character.CharacterVerse>>();
			foreach (Waxuquerque.Character.CharacterVerse cv in control)
			{
				List<Waxuquerque.Character.CharacterVerse> list;
				...
				{
					list = new List<Waxuquerque.Character.CharacterVerse> { cv };
			...
			foreach (BCVRef bcvRef in new SortedSet<BCVRef>(fcbhDictionary.Keys.Union(controlDictionary.Keys)))
			{
				...
				List<Waxuquerque.Character.CharacterVerse> controlList;
				...
				{
					if (hasControl)
						foreach (Waxuquerque.Character.CharacterVerse cv in controlList)
						{
